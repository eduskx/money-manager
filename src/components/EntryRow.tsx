...
//
// Excel-Verhalten fürs Betragsfeld: Ist die Zeile über eine Formel entstanden
// (z. B. „=13+3"), zeigt das Feld im Ruhezustand nur das Ergebnis („16,00").
// Beim Reinklicken erscheint wieder die Formel, sodass man nachvollziehen und
// weiterrechnen kann. Beim Verlassen (onBlur) oder mit Enter wird gespeichert.
export function EntryRow({
  ...
  const resultText = formatAmountInput(amount); // Anzeige im Ruhezustand
  const currentExpr = formula ?? resultText; // Anzeige beim Bearbeiten
  ...
  function commitAmount() {
    if (!editing) return; // schon gespeichert / kein aktiver Edit
    setEditing(false);
  ...
  function amountKey(e: KeyboardEvent<HTMLInputElement>) {
    if (e.key === "Enter") {
      ...
      setEditing(false); // Änderung verwerfen -> zurück zum Ergebnis
    }
  }
    ...
    <div className="group flex min-w-0 items-center gap-1">
      <div className="flex min-w-0 flex-1 items-center gap-1">
        ...
        <input
          ...
          className="w-20 shrink-0 rounded-md border border-transparent bg-transparent px-2 py-2 text-right text-sm text-ink tabular-nums outline-none hover:border-line focus:border-accent focus:bg-surface"
        />
      </div>
